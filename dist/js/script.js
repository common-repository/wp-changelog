jQuery( document ).ready( function ($) {
	...
	function tableSorting(){
		...
		var sortOptions = {
			valueNames: ['id', 'date', 'action', 'name', 'ver', 'type', 'status']
		};
		...
		var filterValue = {type:"all",action:"all",status:"all"}
		...
		$(".list-filter").on("change",function(){
			...
			//"reset filter" link controller
			if ( ( filterValue["type"] == "all" ) && ( filterValue["action"] == "all" ) && ( filterValue["status"] == "all" ) && (!$("#changelog-list input.search").val()) ){
				$(".wbl_reset").removeClass("show");
			} else {
				$(".wbl_reset").addClass("show");
			}
		...
		function reFilter(opt) {
			...
			if ( opt == "reset" ) {
				filterValue = {type:"all",action:"all",status:"all"}
			}
				sortedlist.filter(function(item){
				//add a new checking for each new filter
				if ( ( (filterValue["type"] == item.values().type)||(filterValue["type"] == "all") )
				  && ( (filterValue["action"] == item.values().action)||(filterValue["action"] == "all") )
				  && ( (filterValue["status"] == item.values().status)||(filterValue["status"] == "all") ) ){
					return true;
				} else {
					return false;
				}
