...
class WordPress_Changelog_Class extends WP_Changelog_Init {
	...
	*/
	function __construct() {
		...
		add_action( 'upgrader_process_complete', 			array( &$this, 'theme_and_plugin_install_and_update_action' ), 	10, 2 ); // Plugins, Themes, translation install and update
	...
	public function wordpress_upgrade_action( $version = null ) {
		...
		if ( ! empty( $version ) ) {
			...
			$this->write_log( $action, 'wordpress/core', 'WordPress', 'wordpress-wordpress_upgrade_action', $version, 'core', 'ok' );
		}
	...
	public function plugin_action( $plugin_file, $network_wide = null ) {
		...
		if ( ! empty( $action ) ) {
			...
			if ( is_array( $plugin_data ) && ! empty( $plugin_data ) ) {
				...
				$this->write_log( $actions[ $action ], $plugin_file, $plugin_data['plugin_name'], $plugin_data['plugin_domain'], $plugin_data['plugin_version'], 'plugin', 'ok' );
			}
		}
	...
	public function theme_and_plugin_install_and_update_action( $instance, $extra ) {
		...
		// Check status
		$status = ! empty( $instance->result ) ? 'ok' : 'error';
		...
		switch ( $extra['type'] ) {
			...
			case 'plugin':
				...
				if ( 'install' === $extra['action'] ) {
					...
					if ( is_array( $plugin_data ) && ! empty( $plugin_data ) ) {
						...
						$this->write_log( 'install', $instance->plugin_info(), $plugin_data['plugin_name'], $plugin_data['plugin_domain'], $plugin_data['plugin_version'], 'plugin', $status );
					}
				}
				...
				if ( 'update' === $extra['action'] ) {
					...
					if ( isset( $extra['bulk'] ) && true === $extra['bulk'] ) {
						...
						if ( isset( $extra['plugins'] ) && ! empty( $extra['plugins'] ) && is_array( $extra['plugins'] ) ) {
							...
							foreach ( $extra['plugins'] as $plugin ) {
								...
								if ( is_array( $plugin_data ) && ! empty( $plugin_data ) ) {
									...
									$this->write_log( 'update', $plugin, $plugin_data['plugin_name'], $plugin_data['plugin_domain'], $plugin_data['plugin_version'], 'plugin', $status );
								}
							}
						}
						...
						if ( is_array( $plugin_data ) && ! empty( $plugin_data ) ) {
							...
							$this->write_log( 'update', $instance->skin->plugin, $plugin_data['plugin_name'], $plugin_data['plugin_domain'], $plugin_data['plugin_version'], 'plugin', $status );
						}
					} else {
						return null;
					}
			...
			case 'theme':
				...
				if ( 'install' === $extra['action'] ) {
					...
					if ( $theme_info ) {
						...
						// Save data
						$this->write_log( 'install', $instance->result['destination_name'], $theme_data['theme_name'], $theme_data['theme_domain'], $theme_data['theme_version'], 'theme', $status );
					}
				}
				...
				if ( 'update' === $extra['action'] ) {
					...
					if ( isset( $extra['bulk'] ) && true === $extra['bulk'] ) {
						...
						if ( isset( $extra['themes'] ) && ! empty( $extra['themes'] ) && is_array( $extra['themes'] ) ) {
							...
							foreach ( $extra['themes'] as $theme ) {
								...
								if ( $theme_info ) {
									...
									// Save data
									$this->write_log( 'update', $theme, $theme_data['theme_name'], $theme_data['theme_domain'], $theme_data['theme_version'], 'theme', $status );
								}
							}
						}
						...
						if ( $theme_info ) {
							...
							// Save data
							$this->write_log( 'update', $instance->result['destination_name'], $theme_data['theme_name'], $theme_data['theme_domain'], $theme_data['theme_version'], 'theme', $status );
						}
					} else {
						return null;
					}
			...
			case 'translation':
				...
				if ( isset( $extra['translations'] ) && is_array( $extra['translations'] ) && ! empty( $extra['translations'] ) ) {

					foreach ( $extra['translations'] as $translation ) {

						// Update core translation
						if ( 'core' === $translation['type'] ) {

							// Save data
							$this->write_log( 'update', 'wordpress/core', 'WordPress', 'wordpress', $translation['version'], 'translation', $status );
						}

						// Update plugin translation
						if ( 'plugin' === $translation['type'] && isset( $translation['slug'] ) ) {

							// Get plugin info
							$plugin_data = get_plugins( '/' . $translation['slug'] );
							...
							if ( is_array( $plugin_data ) && ! empty( $plugin_data ) ) {
								...
								foreach ( $plugin_data as $key => $value ) {
									...
									// Save data
									$this->write_log( 'update', $translation['slug'] .'/' . $key, $value['Name'], $value['DomainPath'], $translation['version'], 'translation', $status );
								}
							}
						}

						// Update plugin translation
						if ( 'theme' === $translation['type'] && isset( $translation['slug'] ) ) {

							// Get theme info
							$theme_info = wp_get_theme( $translation['slug'] );
							...
							if ( $theme_info ) {
								...
								// Save data
								$this->write_log( 'update', $translation['slug'], $theme_data['theme_name'], $theme_data['theme_domain'], $translation['version'], 'translation', $status );
							}
						}
					}
				}
	...
	public function delete_themes_action( $transient ) {
		...
		if ( $theme ) {
			...
			// Save data
			$this->write_log( 'delete', $slug, $theme->name, $theme->domain, $theme->version, 'theme', 'ok' );
		}
	...
	public function switch_theme_action( $new_name, $new_theme, $old_theme ) {
		...
		if ( $new_theme_info && $old_theme_info ) {
			...
			$this->write_log( 'deactivate', $old_theme_info->file, $old_theme_info->name, $old_theme_info->domain, $old_theme_info->version, 'theme', 'ok' );

			// Save new theme status
			$this->write_log( 'activate', $new_theme_info->file, $new_theme_info->name, $new_theme_info->domain, $new_theme_info->version, 'theme', 'ok' );
		}
	 ...
	 * @return void.
	 */
	public static function write_log( $action, $file, $name, $domain = null, $version, $type, $status ) {
	...
	public static function setup() {
		...
		// Update options
		update_option( 'wordpress_changelog_version', self::$version );

		/**
		 * Write core, plugin and theme statuses
		 */
		$count_rows = $wpdb->get_var( "SELECT COUNT(*) FROM $table_name" );
